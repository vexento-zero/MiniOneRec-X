    ...
    for split in ["train", "valid", "test"]:
        ...
        if os.path.exists(split_file):
            with open(split_file, "r") as f:
                ...
                splits[split] = [
                    line.strip().split("\t") for line in lines if line.strip()
                ]
...
def create_item_info_file(
    items: Dict[str, Dict], item_to_semantic: Dict[str, List], output_path: str
):
...
def convert_interactions_to_csv(
    splits: Dict[str, List],
    items: Dict[str, Dict],
    item_to_semantic: Dict[str, List],
    output_dir: str,
    ...
    for split_name, split_data in splits.items():
        ...
        for line in split_data:
            ...
            # Target item title
            target_title = items.get(str(target_item_id), {}).get(
                "title", f"Item_{target_item_id}"
            )
            ...
            if split_name == "train" and keep_longest_only:
                ...
                if user_id not in user_to_longest or sequence_length > len(
                    user_to_longest[user_id]["history_item_id"]
                ):
        ...
        if (
            split_name == "valid"
            and max_valid_samples is not None
            and len(rows) > max_valid_samples
        ):
            rows = random.sample(rows, max_valid_samples)
            print(
                f"  Sampled {max_valid_samples} from {original_count} validation samples"
            )
        elif (
            split_name == "test"
            and max_test_samples is not None
            and len(rows) > max_test_samples
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert dataset (Office_Products/Industrial_and_Scientific) to MiniOneRec format with semantic IDs"
    )
    ...
    parser.add_argument(
        "--output_dir", type=str, help="Output directory for MiniOneRec format data"
    )
    parser.add_argument(
        "--category",
        type=str,
    ...
    info_file = os.path.join(args.output_dir, "info", f"{args.category}.txt")
    create_item_info_file(
        dataset_data["items"], dataset_data["item_to_semantic"], info_file
    )
    ...
    print(
        f"You can now use these files with MiniOneRec training scripts by setting category='{args.category}'"
    )
