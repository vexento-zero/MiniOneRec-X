...
class ReReTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            processing_class = AutoTokenizer.from_pretrained(
                self.base_model, padding_side="left"
            )
            ...
            with unwrap_model_for_generation(
                ...
                if self.beam_search:
                    ...
                    if self.dynamic_sampling:
                        ...
                        prompt_completion_ids = unwrapped_model.generate(
                            extended_prompt_ids,
                            attention_mask=extended_prompt_mask,
                            generation_config=self.generation_config,
                            logits_processor=self.logits_processor,
                        )
                        ...
                        def select_completion(completions, target):
                            ...
                            if target in completions:
                                selected.extend(
                                    [target]
                                    * min(
                                        completion_times[target], self.num_generations
                                    )
                                )
                        ...
                        for i in range(
                            0,
                            len(extended_completions_text),
                            int(self.num_generations * 1.5),
                        ):
                            selected_completion.extend(
                                select_completion(
                                    extended_completions_text[
                                        i : i + int(self.num_generations * 1.5)
                                    ],
                                    extended_targets[i],
                                )
                            )
                        # print(f"selected_completion: {len(selected_completion)}")
                        selected_completion_ids = self.processing_class(
                            selected_completion,
                            return_tensors="pt",
                            padding=True,
                            padding_side="right",
    ...
    def compute_loss(
        ...
        if self.dapo:
            ...
            s_score = torch.exp(
                (per_token_ratio * completion_mask).sum(dim=1)
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if not hasattr(self, "g_metrics"):
            self.g_metrics = defaultdict(list)

        for k, v in metrics.items():
            self.g_metrics[k].append(v)

        if len(list(self.g_metrics.values())[0]) % 10 == 0:
            self.plot_rewards(self.g_metrics, self.args.output_dir)
    ...
    def plot_rewards(self, metrics_data, output_dir, avg_step=10):
        os.makedirs(output_dir, exist_ok=True)
        
        fig, ax = plt.subplots(1, 1, figsize=(12, 6))
        
        reward_keys = [k for k in metrics_data.keys() if k.startswith("reward")]
        colors = plt.cm.tab10(np.linspace(0, 1, len(reward_keys)))
        
        for idx, key in enumerate(reward_keys):
            values = metrics_data[key]
            if len(values) > avg_step:
                # 计算移动平均
                avg_values = []
                for i in range(0, len(values), avg_step):
                    end_idx = min(i + avg_step, len(values))
                    avg_values.append(sum(values[i:end_idx]) / (end_idx - i))
                
                x = [i * avg_step + avg_step//2 for i in range(len(avg_values))]
                ax.plot(
                    x,
                    avg_values,
                    color=colors[idx],
                    linewidth=2,
                    label=f"{key} (avg over {avg_step} steps)",
                )
        
        ax.set_xlabel("Step", fontsize=12)
        ax.set_ylabel("Value", fontsize=12)
        ax.set_title("Reward Metrics (Moving Average)", fontsize=14, fontweight="bold")
        ...
        save_path = os.path.join(
            output_dir, f"rewards-plot-{self.accelerator.process_index}.png"
        )
