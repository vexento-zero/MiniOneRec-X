...
def balanced_kmeans_level_constrained(
    ...
    print(
        f"[Time] balanced_kmeans_level_constrained (K={K}): {time.time() - start_time:.2f}s"
    )
...
def residual_kmeans_constrained(
    ...
    for l in range(L):
        ...
        seed_l = (
            ...
            else int(np.random.RandomState(random_state + l).randint(0, 2**31 - 1))
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--root", type=str, default="./data/Amazon", help="Root directory for data"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--k", type=int, default=256, help="Number of clusters per level"
    )
...
if __name__ == "__main__":
    ...
    codebook_path = os.path.join(
        output_dir, f"{args.dataset}.codebooks_constrained.npz"
    )
    np.savez_compressed(
        codebook_path, **{f"codebook_{i}": cb for i, cb in enumerate(codebooks)}
    )
    ...
    print(
        f"- Final reconstruction error (MSE): {np.mean((embeddings - recon) ** 2):.6f}"
    )

    # Deduplication statistics
    codes_str = codes_df.with_columns(
        pl.col("codes")
        .map_elements(lambda x: ",".join(map(str, x)), return_dtype=pl.Utf8)
        .alias("codes_str")
    )
    duplicates = (
        codes_str.group_by("codes_str")
        .count()
