...
class Trainer(object):
    ...
    def fit(self, data):
        ...
        for epoch_idx in tqdm(
            range(self.epochs), ncols=100, desc=set_color("Training", "pink")
        ):
            # train
            train_loss, train_recon_loss = self._train_epoch(data, epoch_idx)
            train_recon_losses.append(train_recon_loss)
            train_quant_losses.append(train_loss - train_recon_loss)

            # eval
            if (epoch_idx + 1) % self.eval_step == 0:
                self._plot_loss_curves(train_recon_losses, train_quant_losses)
    ...
    def _plot_loss_curves(self, recon_losses, quant_losses, avg_window=5):
        start = 100
        ...
        recon_losses = recon_losses[start:]
        quant_losses = quant_losses[start:]

        recon_losses = moving_average(recon_losses, avg_window)
        quant_losses = moving_average(quant_losses, avg_window)

        fig, ax = plt.subplots(1, 1, figsize=(8, 4))

        epochs = range(
            start + (avg_window - 1 if len(recon_losses) >= avg_window else 0),
            start
            + (avg_window - 1 if len(recon_losses) >= avg_window else 0)
            + len(recon_losses),
        )
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.set_title(f"Training Loss (avg over {avg_window})")
        ...
        plt.savefig(
            os.path.join(self.ckpt_dir, "loss_curves.png"), dpi=150, bbox_inches="tight"
        )
        plt.close()
