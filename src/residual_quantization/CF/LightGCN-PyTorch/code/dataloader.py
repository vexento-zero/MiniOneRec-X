...
class Loader(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/gowalla"):
        ...
        trainUniqueUsers_list = []
        trainUser_list = []
        trainItem_list = []
        testUniqueUsers_list = []
        testUser_list = []
        testItem_list = []
        ...
        for uid, items in user_items_dict.items():
            if len(items) < 2:
                # 交互太少，全部放入训练集
                for iid in items:
                    trainUser_list.append(uid)
                    trainItem_list.append(iid)
                trainUniqueUsers_list.append(uid)
            else:
                # 随机选取一个物品作为测试集
                ...
                for iid in test_items:
                    testUser_list.append(uid)
                    testItem_list.append(iid)
                testUniqueUsers_list.append(uid)

        self.trainUniqueUsers = np.array(trainUniqueUsers_list)
        self.trainUser = np.array(trainUser_list)
        self.trainItem = np.array(trainItem_list)
        self.testUniqueUsers = np.array(testUniqueUsers_list)
        self.testUser = np.array(testUser_list)
        self.testItem = np.array(testItem_list)
        ...
        print(
            f"{world.dataset} Sparsity : {(self.traindataSize + self.testDataSize) / self.n_user / self.m_item}"
        )
    ...
    # def __init__(self, config=world.config, path="../data/gowalla"):
    #     path = "/ssd/hust-tangxi/workspace/MOR/src/residual_quantization/CF/LightGCN-PyTorch/data/amazon-book"
