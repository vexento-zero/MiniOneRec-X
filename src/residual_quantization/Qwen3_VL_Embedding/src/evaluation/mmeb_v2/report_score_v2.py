...
EXPERIMENTS = [
    {
        "path": r"results/evaluation/mmeb_v2/Qwen3-VL-Embedding-8B",
        "metadata": {
            "model_name": r"Qwen3-VL-Embedding-8B",
            "model_backbone": "qwen3vl",
            "model_size": "8B parameters",
            "embedding_dimension": 4096,
            "max_length_tokens": 32768,
            "model_release_date": "2026-01-08",
...
for experiment in EXPERIMENTS:
    ...
    final_output = {"metadata": final_metadata, "metrics": current_experiment_scores}
    ...
    for modality in modalities:
        ...
        if collected_metric_values:
            all_metric_values.extend(collected_metric_values)
            average_score = sum(collected_metric_values) / len(collected_metric_values)
            print(
                f"      Average of {modality.upper()}\t- {main_metric_key}:\t{average_score:.4f} (from {len(collected_metric_values)} datasets)"
            )
        else:
            print(
                f"      Average of {modality.upper()}\t-  {main_metric_key}:\tN/A (no valid scores found)"
            )

        if datasets_missing_score_file:
            print(f"      Datasets with missing/errored score files:")
            for ds_status in datasets_missing_score_file:
                print(f"        - {ds_status}")
        if datasets_file_found_metric_missing:
            print(
                f"      Score files found but main metric ('{main_metric_key}') missing/invalid:"
            )
            for ds_status in datasets_file_found_metric_missing:
                print(f"        - {ds_status}")

    if all_metric_values:
        print(
            f"      All Average:\t{sum(all_metric_values)/len(all_metric_values):.4f} (from {len(all_metric_values)} datasets)"
        )
