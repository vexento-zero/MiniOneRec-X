...
class RankingMetrics:
    ...
    def hit_at_k(self, prediction, true_labels, k):
        ...
        if k == 0:
            return 0.0
        relevant_hits, _ = self._get_relevant_hits_and_predicted_k(
            prediction, true_labels, k
        )
    ...
    def average_precision_at_k(self, prediction, true_labels, k):
        ...
        return score / min(
            len(true_labels), k
        )  # Normalize by min of total relevant or K for AP@K
    ...
    def mean_average_precision_at_k(self, test_cases_for_map_mrr, k_val):
        ...
        for case in test_cases_for_map_mrr:
            ...
            ap_scores.append(
                self.average_precision_at_k(prediction, true_labels_list, k_val)
            )
    ...
    def ndcg_at_k(self, prediction, true_labels, k, rel_scores, form="linear"):
        ...
        if rel_scores is None:
            ...
            # Graded relevance
            if (
                not isinstance(true_labels, list)
                or not isinstance(rel_scores, list)
                or len(true_labels) != len(rel_scores)
            ):
                raise ValueError(
                    "If rels is provided, label must be a list of the same length."
                )
        ...
        idcg_score = (
            dcg(ideal_relevance, form) if ideal_relevance else 1
    ...
    def evaluate(self, test_cases):
        ...
        for case_idx, case in enumerate(test_cases):
            ...
            true_labels = list(
                set(true_labels)
            )  # Ensure unique true labels and convert to list for consistency
            ...
            for k in self.k_list:
                if "precision" in self.metric_list:
                    metric_results_accumulators["precision"][k].append(
                        self.precision_at_k(prediction, true_labels, k)
                    )
                if "recall" in self.metric_list:
                    metric_results_accumulators["recall"][k].append(
                        self.recall_at_k(prediction, true_labels, k)
                    )
                if "hit" in self.metric_list:
                    metric_results_accumulators["hit"][k].append(
                        self.hit_at_k(prediction, true_labels, k)
                    )
                if "f1" in self.metric_list:
                    metric_results_accumulators["f1"][k].append(
                        self.f1_at_k(prediction, true_labels, k)
                    )
                if "ndcg" in self.metric_list:
                    rel_scores = case["rel_scores"]
                    metric_results_accumulators["ndcg_linear"][k].append(
                        self.ndcg_at_k(
                            prediction, true_labels, k, rel_scores, form="linear"
                        )
                    )
                    metric_results_accumulators["ndcg_exponential"][k].append(
                        self.ndcg_at_k(
                            prediction, true_labels, k, rel_scores, form="exponential"
                        )
                    )
        ...
        if "map" in self.metric_list:
            for k_val in self.k_list:
                ap_scores = [
                    self.average_precision_at_k(
                        case["prediction"], case["label"], k_val
                    )
                    for case in processed_test_cases_for_map_mrr
                ]
                score_dict[f"map@{k_val}"] = (
        ...
        if "mrr" in self.metric_list:
            for k_val in self.k_list:
                ...
                score_dict[f"mrr@{k_val}"] = (
                    float(np.mean(rr_scores)) if rr_scores else 0.0
                )
