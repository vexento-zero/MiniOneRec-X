...
class CSVBaseDataset(BaseDataset):
    def __init__(
        ...
        # print(f"CSV read time: {time.time() - start_time}")
...
"""
        ...
        history = {"input": input_text, "output": target_item_with_prefix}
...
"""
    ...
    def get_history(self, row):
        ...
        return {
            "input": f"Can you predict the next possible item the user may expect, given the following chronological interaction history: {history}",  # f"The user has interacted with items {history} in chronological order. Can you predict the next possible item that the user may expect?",
            "output": target_item + "\n",
            "dedup": target_item_sid == last_history_item_sid,
        }
...
class RLTitle2SidDataset(JSONBaseDataset):
    def __init__(
        self, item_file, index_file, sample=-1, seed=0, category="", dedup=False
    ):
        """
        ...
        for description, sid in self.description2sid.items():
            self.data.append(
                {"task": "description2sid", "input": description, "output": sid}
            )
        ...
        if self.dedup and "history_item_id" in row:
            try:
                ...
                last_history_item_id = (
                    str(history_item_id[-1]) if history_item_id else None
                )
                is_duplicate = target_item_id == last_history_item_id
            except:
                is_duplicate = False
...
class PreferenceSFTDataset(BaseDataset):
    ...
    def get_history(self, row_data):
        ...
        target_sid = (
            target_semantic_ids[0] if target_semantic_ids else str(target_item_id)
        )
...
"""
        ...
        if (
            not history_and_pref["history_semantic_ids"]
            or not history_and_pref["target_sid"]
        ):
            return None
...
"""
        ...
        # Skip empty histories or missing targets
        if (
            not input_and_target["history_semantic_ids"]
            or not input_and_target["target_sid"]
        ):
            return None
